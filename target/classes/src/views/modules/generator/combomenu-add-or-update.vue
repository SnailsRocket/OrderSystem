    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.comboId) {
            this.$http({
              url: this.$http.adornUrl(`/generator/combomenu/info/${this.dataForm.comboId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.menuId = data.comboMenu.menuId
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/combomenu/${!this.dataForm.comboId ? 'save' : 'update'}`),
              method: 'post',
